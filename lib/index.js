...
const defaultMaxStatusCode = 299
const defaultUserAgent = 'liscio-undici'
...
const makeRequest = (client, opts) => {
    ...
    return async requestArgs => {
		...
		*/
        ...
        const maxStatus = maxStatusCode || defaultMaxStatusCode

        if (!disableRejectGtMaxStatusCode && statusCode > maxStatus) {
...
const httpClientFactory = clientOptions => {
    ...
    const httpClient = client
        ? client
        : createHttpClient(baseUrl, undiciOptions)
	...
	*/
    ...
    if (responseInterceptors) {
        _responseInterceptors =
            _responseInterceptors.concat(responseInterceptors)
    }
